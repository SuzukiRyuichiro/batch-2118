...
def check_all
  ...
  check('ruby version') do
    if RUBY_VERSION == REQUIRED_RUBY_VERSION
      ...
      details = `type -a ruby`
      [false,
       "Your default ruby version is #{RUBY_VERSION}, but should be #{REQUIRED_RUBY_VERSION}. Did you run `rbenv global #{REQUIRED_RUBY_VERSION}`?\n#{details}---"]
    end
  end
  ...
  check('git/Github email matching') do
    ...
    [ok,
     ok ? 'git email is included in Github emails' : "Add #{git_email} to your GitHub account or update your git global settings"]
  end
